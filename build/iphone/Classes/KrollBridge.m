...
NSString * _titaniumnightclubNightClub$ModuleRequireFormat = @"(function(exports){"
...
@implementation _titaniumnightclubNightClubObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_titaniumnightclub);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create _titaniumnightclubNightClub global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "_titaniumnightclubNightClub" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_titaniumnightclub = [[_titaniumnightclubNightClubObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_titaniumnightclub];
	
	NSString *_titaniumnightclubNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _titaniumnightclubNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_titaniumnightclub valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:_titaniumnightclubNightClub$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.titaniumnightclub.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.titaniumnightclub.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
