...
//Rare exceptions to indicate a bug in the _titaniumnightclub code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_titaniumnightclub = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_titaniumnightclub]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.titaniumnightclub.module"
