...
createGallery = function(param) 
{
	...
	var defaultPortraitWidth=320;
	var defaultPortraitHeight=480;
	var defaultLandScapeWidth=480;
	var defaultLandScapeHeight=320;
	 ...
	 var startFBShare=function()
        {
        	if(Ti.Facebook.loggedIn) 
				...
				else 
				{
					...
					Titanium.Facebook.addEventListener('login', logedin);
	...
	var createImageWin = function(index) 
	{
		...
		if(VARS._platform!=VARS._android)
		{
			...
			toolbar = Ti.UI.iOS.createToolbar({
				items : [share, flexSpace, leftButton, flexSpace, rightButton, flexSpace, download],
				bottom : 0,
				borderTop : true,
				borderBottom : true,
				barColor : '#000'
